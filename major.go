...
const (
	...
	// Major type 0: An unsigned integer in the range 0..264-1 inclusive. The value of the encoded item is the argument itself. For example, the integer 10 is denoted as the one byte 0b000_01010 (major type 0, additional information 10). The integer 500 would be 0b000_11001 (major type 0, additional information 25) followed by the two bytes 0x01f4, which is 500 in decimal.
	CBORMajor0NonNegative CBORMajor = iota
	// Major type 1: A negative integer in the range -264..-1 inclusive. The value of the item is -1 minus the argument. For example, the integer -500 would be 0b001_11001 (major type 1, additional information 25) followed by the two bytes 0x01f3, which is 499 in decimal.
	CBORMajor1Negative
	...
	CBORMajor5Map
	// Major type 6: A tagged data item ("tag") whose tag number, an integer in the range 0..264-1 inclusive, is the argument and whose enclosed data item (tag content) is the single encoded data item that follows the head. See Section 3.4.
	CBORMajor6Tag
	// Major type 7: Floating-point numbers and simple values, as well as the "break" stop code. See Section 3.3.
	CBORMajor7Simple

	CBORMajorUnknown CBORMajor = 0xff
)
